...
import {
  Globe, Mail, Shield, Smartphone, Zap, Clock, CheckCircle2, ArrowRight,
...
const testimonials = [
  { name: 'Marco Rossi', role: 'Ägare, Trattoria Bella', text: 'Våra gäster hittar oss enkelt och bordsbokningarna har ökat med 40% sedan vi bytte till WebPro. Fantastisk service!', stars: 5 },
  { name: 'Emma Lindqvist', role: 'Grundare, Nordbygg AB', text: 'Som entreprenör har jag ingen tid med teknik. WebPro sköter allt och deras support är fantastisk. Jag bara ringer och de fixar det.', stars: 5 },
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20">
            <a href="#" className="flex items-center gap-2.5 group">
              <div className="w-9 h-9 rounded-lg bg-amber-600 flex items-center justify-center"><Globe className="w-5 h-5 text-white" /></div>
              <span className="text-xl font-bold text-stone-900 group-hover:text-amber-700 transition-colors">Web<span className="text-amber-600">Pro</span></span>
            </a>
            <div className="hidden md:flex items-center gap-6">
              {navLinks.map(l => <a key={l.href} href={l.href} className="text-sm font-medium text-stone-600 hover:text-amber-700 transition-colors">{l.label}</a>)}
              ...
              <a href="#kontakt"><Button className="bg-amber-600 hover:bg-amber-700 text-white rounded-full px-6">Kom igång</Button></a>
            </div>
        ...
        {mobileOpen && (
          <div className="md:hidden bg-white border-t border-stone-200 shadow-lg">
            <div className="px-4 py-4 space-y-3">
              {navLinks.map(l => <a key={l.href} href={l.href} className="block py-2 text-stone-700 font-medium hover:text-amber-700" onClick={() => setMobileOpen(false)}>{l.label}</a>)}
              ...
              <a href="#kontakt" onClick={() => setMobileOpen(false)}><Button className="w-full bg-amber-600 hover:bg-amber-700 text-white rounded-full mt-2">Kom igång</Button></a>
            </div>
          </div>
        )}
      </nav>
      ...
      <main className="flex-1">
        ...
        <section id="tjanster" className="py-20 sm:py-28 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-2xl mx-auto mb-16">
              <Badge className="mb-4 bg-amber-100 text-amber-800 border-amber-200">Våra tjänster</Badge>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {services.map(s => { const Icon = s.icon; return (
                <Card key={s.title} className="group border-stone-200 hover:border-amber-300 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                  <CardHeader><div className="w-12 h-12 rounded-xl bg-amber-100 flex items-center justify-center mb-3 group-hover:bg-amber-600 transition-colors"><Icon className="w-6 h-6 text-amber-700 group-hover:text-white transition-colors" /></div><CardTitle className="text-xl text-stone-900">{s.title}</CardTitle></CardHeader>
        ...
        <section id="branscher" className="py-20 sm:py-28 bg-stone-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-2xl mx-auto mb-16">
              <Badge className="mb-4 bg-amber-100 text-amber-800 border-amber-200">Branscher</Badge>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {industries.map(ind => { const Icon = ind.icon; return (
                <Card key={ind.name} className="group border-stone-200 hover:border-amber-300 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 text-center">
                  <CardHeader className="items-center pb-2"><div className="w-14 h-14 rounded-2xl bg-amber-100 flex items-center justify-center mb-2 group-hover:bg-amber-600 transition-colors"><Icon className="w-7 h-7 text-amber-700 group-hover:text-white transition-colors" /></div><CardTitle className="text-lg text-stone-900">{ind.name}</CardTitle></CardHeader>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 text-center text-white">
              {[['99.9%', 'Upptid'], ['< 2s', 'Laddtid'], ['24/7', 'Övervakning'], ['30 dag', 'Backup-historik']].map(([v, l]) => (
                <div key={l}><div className="text-3xl sm:text-4xl font-bold">{v}</div><div className="text-amber-200 text-sm mt-1">{l}</div></div>
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="py-20 sm:py-28 bg-stone-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-2xl mx-auto mb-16">
              <Badge className="mb-4 bg-amber-100 text-amber-800 border-amber-200">Kundröster</Badge>
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {testimonials.map(t => (
                <Card key={t.name} className="border-stone-200 hover:shadow-lg transition-shadow duration-300">
                  <CardHeader><div className="flex gap-0.5 mb-2">{Array.from({ length: t.stars }).map((_, i) => <Star key={i} className="w-5 h-5 fill-amber-400 text-amber-400" />)}</div></CardHeader>
        ...
        <section id="om-oss" className="py-20 sm:py-28 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div>
                <Badge className="mb-4 bg-amber-100 text-amber-800 border-amber-200">Om oss</Badge>
                <h2 className="text-3xl sm:text-4xl font-bold text-stone-900 mb-6">Vi förstår svenska företag — och webben</h2>
                <div className="space-y-4 text-stone-600 leading-relaxed">
                  <p>WebPro grundades med en enkel idé: svenska företag förtjänar bättre hemsidor. För ofta ser vi fantastiska företag med dåliga, långsamma eller osäkra hemsidor som kostar för mycket.</p>
                ...
                <div className="mt-8 flex flex-wrap gap-6">
                  {[{ icon: Shield, t: 'Säker hosting', s: 'Imunify360 + WAF' }, { icon: Globe, t: '.SE-domäner', s: 'Certifierade registrarer' }, { icon: Headphones, t: 'Svensk support', s: 'Personlig & snabb' }].map(x => { const Icon = x.icon; return (
                    <div key={x.t} className="flex items-center gap-2"><div className="w-10 h-10 rounded-lg bg-amber-100 flex items-center justify-center"><Icon className="w-5 h-5 text-amber-700" /></div><div><p className="font-semibold text-stone-900 text-sm">{x.t}</p><p className="text-xs text-stone-500">{x.s}</p></div></div>
                  ); })}
                </div>
              </div>
              <div className="relative">
                <div className="aspect-[4/3] rounded-2xl bg-gradient-to-br from-amber-200 via-amber-100 to-stone-200 flex items-center justify-center"><div className="text-center p-8"><Server className="w-20 h-20 text-amber-600 mx-auto mb-4" /><p className="text-amber-800 font-semibold text-lg">Professionell infrastruktur</p><p className="text-amber-700 text-sm mt-1">WHM/cPanel · LiteSpeed · CloudLinux</p></div></div>
                <div className="absolute -bottom-6 -right-6 w-32 h-32 bg-amber-600 rounded-2xl flex items-center justify-center text-white shadow-lg"><div className="text-center"><div className="text-3xl font-bold">5+</div><div className="text-xs text-amber-200">Års erfarenhet</div></div></div>
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="grid lg:grid-cols-2 gap-16">
              <div>
                ...
                <div className="space-y-6">
                  {[{ icon: Mail, t: 'E-post', d: 'hej@webpro.se' }, { icon: Clock, t: 'Svarstid', d: 'Inom 24 timmar vardagar' }, { icon: Search, t: 'Gratis konsultation', d: 'Vi analyserar din nuvarande närvaro' }].map(x => { const Icon = x.icon; return (
                    <div key={x.t} className="flex items-start gap-4"><div className="w-12 h-12 rounded-xl bg-amber-100 flex items-center justify-center shrink-0"><Icon className="w-6 h-6 text-amber-700" /></div><div><p className="font-semibold text-stone-900">{x.t}</p><p className="text-stone-600">{x.d}</p></div></div>
                  ); })}
                </div>
              </div>
              <Card className="border-stone-200">
                ...
                <CardContent>
                  <form onSubmit={handleContact} className="space-y-5">
                    ...
                    <Button type="submit" className="w-full bg-amber-600 hover:bg-amber-700 text-white rounded-full h-12 text-base">{contactSent ? 'Tack! Vi hör av oss snart.' : 'Skicka förfrågan'}{!contactSent && <ArrowRight className="w-4 h-4 ml-2" />}</Button>
                  </form>
                </CardContent>
              </Card>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            <div className="sm:col-span-2 lg:col-span-1">
              <div className="flex items-center gap-2.5 mb-4"><div className="w-9 h-9 rounded-lg bg-amber-600 flex items-center justify-center"><Globe className="w-5 h-5 text-white" /></div><span className="text-xl font-bold text-white">Web<span className="text-amber-400">Pro</span></span></div>
          ...
          <div className="border-t border-stone-800 pt-8 text-sm text-center">&copy; {new Date().getFullYear()} WebPro. Alla rättigheter förbehållna.</div>
        </div>
      </footer>
    </div>
  );
...
function LoginPage({ onLogin, onBack }: { onLogin: () => void; onBack: () => void }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-stone-900 via-stone-800 to-amber-900 px-4">
      <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 50% 50%, rgba(217,119,6,0.3) 0%, transparent 60%)' }} />
      <Card className="w-full max-w-md relative z-10 shadow-2xl border-stone-200">
        <CardHeader className="text-center space-y-4 pb-2">
          <div className="flex items-center justify-center gap-2.5"><div className="w-10 h-10 rounded-lg bg-amber-600 flex items-center justify-center"><Globe className="w-6 h-6 text-white" /></div><span className="text-2xl font-bold text-stone-900">Web<span className="text-amber-600">Pro</span></span></div>
        ...
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div><label className="block text-sm font-medium text-stone-700 mb-1.5">E-post</label><Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@webpro.se" className="h-11" required /></div>
            ...
            <Button type="submit" className="w-full bg-amber-600 hover:bg-amber-700 text-white h-11 rounded-lg" disabled={loading}>{loading ? 'Loggar in...' : 'Logga in'}</Button>
          </form>
          <div className="mt-4 text-center text-xs text-stone-400">Standard: admin@webpro.se / admin123</div>
          <button onClick={onBack} className="mt-6 w-full text-center text-sm text-stone-500 hover:text-amber-700 transition-colors">&larr; Tillbaka till hemsidan</button>
        </CardContent>
      </Card>
    </div>
  );
    ...
    <div className="min-h-screen bg-stone-100 flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-stone-900 text-white transform transition-transform duration-200 lg:translate-x-0 lg:static ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center gap-2.5 px-6 h-16 border-b border-stone-800">
          <div className="w-8 h-8 rounded-lg bg-amber-600 flex items-center justify-center"><Globe className="w-4 h-4 text-white" /></div>
          <span className="text-lg font-bold">Web<span className="text-amber-400">Pro</span></span>
        </div>
        <nav className="p-4 space-y-1">
          {navItems.map(n => { const Icon = n.icon; return (
            <button key={n.id} onClick={() => { setSection(n.id); setSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${section === n.id ? 'bg-amber-600 text-white' : 'text-stone-400 hover:text-white hover:bg-stone-800'}`}>
...
function OverviewSection({ stats, customers, messages }: { stats: Stats; customers: Customer[]; messages: ContactMsg[] }) {
  const cards = [
    { label: 'Totalt kunder', value: stats.totalCustomers, icon: Users, color: 'bg-amber-100 text-amber-700' },
        ...
        <Card className="border-stone-200"><CardHeader><CardTitle className="text-lg">Senaste meddelanden</CardTitle></CardHeader><CardContent>
          <div className="space-y-3">{stats.recentMessages.map(m => (
            <div key={m.id} className="flex items-start gap-3 py-2 border-b border-stone-100 last:border-0">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.read ? 'bg-stone-100' : 'bg-amber-100'}`}><MessageSquare className={`w-4 h-4 ${m.read ? 'text-stone-400' : 'text-amber-600'}`} /></div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
        ...
        <Button onClick={openAdd} className="bg-amber-600 hover:bg-amber-700 text-white"><Plus className="w-4 h-4 mr-2" />Ny kund</Button>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => { setShowAdd(false); setEditId(null); }}>
          <Card className="w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3 pt-2"><Button onClick={saveCustomer} className="bg-amber-600 hover:bg-amber-700 text-white flex-1">{editId ? 'Spara ändringar' : 'Skapa kund'}</Button><Button variant="outline" onClick={() => { setShowAdd(false); setEditId(null); }}>Avbryt</Button></div>
            </CardContent>
          </Card>
        </div>
      )}
...
function ProjectsSection({ projects, customers, onRefresh }: { projects: Project[]; customers: Customer[]; onRefresh: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end"><Button onClick={() => setShowAdd(true)} className="bg-amber-600 hover:bg-amber-700 text-white"><Plus className="w-4 h-4 mr-2" />Nytt projekt</Button></div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <Card className="w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3 pt-2"><Button onClick={saveProject} className="bg-amber-600 hover:bg-amber-700 text-white flex-1">Skapa projekt</Button><Button variant="outline" onClick={() => setShowAdd(false)}>Avbryt</Button></div>
            </CardContent>
          </Card>
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
        ...
        <Button onClick={() => setShowAdd(true)} className="bg-amber-600 hover:bg-amber-700 text-white"><Plus className="w-4 h-4 mr-2" />Ny faktura</Button>
      </div>
      <Card className="border-stone-200 overflow-hidden"><div className="overflow-x-auto">
        <table className="w-full text-sm">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <Card className="w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3 pt-2"><Button onClick={saveInvoice} className="bg-amber-600 hover:bg-amber-700 text-white flex-1">Skapa faktura</Button><Button variant="outline" onClick={() => setShowAdd(false)}>Avbryt</Button></div>
            </CardContent>
          </Card>
        </div>
      )}
...
function MessagesSection({ messages, onRefresh }: { messages: ContactMsg[]; onRefresh: () => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {messages.map(m => (
        <Card key={m.id} className={`border-stone-200 transition-all ${!m.read ? 'border-l-4 border-l-amber-500 bg-amber-50/30' : ''}`}>
          <CardContent className="p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex gap-1 shrink-0">
                <button onClick={() => toggleRead(m)} className="p-1.5 text-stone-400 hover:text-amber-600 rounded" title={m.read ? 'Markera oläst' : 'Markera läst'}>{m.read ? <Mail className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}</button>
    ...
    <div className="max-w-md space-y-6">
      <Card className="border-stone-200"><CardHeader><CardTitle>Ändra lösenord</CardTitle></CardHeader><CardContent>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <Button type="submit" className="bg-amber-600 hover:bg-amber-700 text-white">{saved ? 'Sparat!' : 'Spara lösenord'}</Button>
        </form>
      </CardContent></Card>
    </div>
  );
...
export default function Home() {
  ...
  const handleLogout = () => { setLoggedIn(false); setView('landing'); document.cookie = 'webpro_session=; max-age=0; path=/'; };
